...
const Login = () => {
    ...
    const handleGoogleClick = () => {
        const clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID || "942388377321-su1u7ofm0ck76pvkiv07ksl8k9esfls6.apps.googleusercontent.com";
        if (!clientId) {
        ...
        const authUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=${responseType}&scope=${encodeURIComponent(scope)}&access_type=${accessType}&state=${state}&prompt=consent`;
    ...
    const handleLinkedinClick = () => {
        ...
        const authUrl = `https://www.linkedin.com/oauth/v2/authorization?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=${responseType}&scope=${encodeURIComponent(scope)}&state=${state}`;
        window.location.href = authUrl;
    };
